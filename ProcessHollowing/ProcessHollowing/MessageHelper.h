...
private:
	...
	static void PrintMethodSpecification()
	{
		...
		std::wcout << L"\t\t\t\te. NtSetContextThread" << std::endl;
		...
		std::wcout << L"\t\t5- The use of NtSetContextThread enhances the ability of anti-malware tools to detect malicious behavior more effectively" << std::endl;
		std::wcout << L"\t\t6- Perform process hollowing with altering the host process's details, including its context and entry point address" << std::endl;
	}

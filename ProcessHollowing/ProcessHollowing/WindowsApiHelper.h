...
typedef NTSTATUS(NTAPI* pNtSetContextThread)(HANDLE hThread, PCONTEXT lpContext);
...
NTSTATUS MyNtSetContextThread(HANDLE hThread, PCONTEXT lpContext) 
{
	if (NtSetContextThread)
	{
		return NtSetContextThread(hThread, lpContext);
	}
